...
from progress import (
    ...
    update_confidence, get_stats, get_review_items, get_difficulty_distribution
)
...
@app.route('/difficulty_distribution', methods=['GET'])
def difficulty_distribution_endpoint():
    try:
        user_id = int(request.args.get('user_id', 1))
        distribution = get_difficulty_distribution(user_id)
    ...
    try:
        ...
        if not force_refresh:
            with closing(_get_connection()) as conn:
                cached = conn.execute(
                    "SELECT words, created_at FROM related_words_cache WHERE user_id = ?", 
                    (user_id,)
                ).fetchone()
                ...
                if cached:
                    ...
                    if datetime.now() - created_at < timedelta(hours=24):
                        import json
                        return {"related_words": json.loads(cached['words'])}
        ...
        try:
            ...
            if response and response.text:
                ...
                if isinstance(related_words, list):
                    ...
                    if new_words:
                        final_words = new_words[:8]
                        
                        # Cache the results
                        with closing(_get_connection()) as conn:
                            conn.execute(
                                "INSERT OR REPLACE INTO related_words_cache (user_id, words, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
                                (user_id, json.dumps(final_words))
                            )
                            conn.commit()
                            
                        return {"related_words": final_words}
        except Exception as e:
            print(f"AI related words failed: {e}")
            import traceback
            traceback.print_exc()
        
        # Fallback: return empty array with message and cache it to prevent continuous retries
        try:
             with closing(_get_connection()) as conn:
                conn.execute(
                    "INSERT OR REPLACE INTO related_words_cache (user_id, words, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
             ...
             print(f"Failed to cache related words fallback: {db_err}")
...
def word_of_day_endpoint():
    """Get word of the day (cached in DB for the day)."""
    try:
        ...
        with closing(_get_connection()) as conn:
            # Check DB for today's word
            row = conn.execute(
                "SELECT word, difficulty FROM word_of_day_history WHERE date = ?", 
                (today_str,)
            ).fetchone()
            
            if row:
                return {"word": row['word'], "difficulty": row['difficulty']}
        
            # Generate new word if not found
            try:
                model = GenerativeModel('gemini-2.5-flash-lite')
                prompt = (
                ...
                if response and response.text:
                    ...
                    # Clean up if it returns JSON or extra format
                    if "{" in word: # Simple check if it returned a JSON object by mistake
                         import json
                         try:
                             j = json.loads(word)
                             if 'word' in j: word = j['word']
                         except: pass
                    ...
                    # Store in DB
                    conn.execute(
                        "INSERT INTO word_of_day_history (date, word, difficulty) VALUES (?, ?, ?)",
                        (today_str, word, difficulty)
                    )
                    conn.commit()
                ...
                # Cache the fallback word too to prevent retrying API today
                try:
                    conn.execute(
                        "INSERT INTO word_of_day_history (date, word, difficulty) VALUES (?, ?, ?)",
                        (today_str, word, 3)
                    ...
                    print(f"Failed to cache fallback: {db_err}")
                
                return {"word": word, "difficulty": 3}
