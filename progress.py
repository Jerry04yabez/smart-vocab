...
def get_difficulty_distribution(user_id: int) -> Dict[int, int]:
    """Get the count of words by difficulty level for a user."""
    with closing(_get_connection()) as conn:
        rows = conn.execute(
            "SELECT difficulty, COUNT(*) as count FROM user_words WHERE user_id=? GROUP BY difficulty",
            (user_id,)
        ).fetchall()
        
        # Initialize all difficulty levels to 0
        ...
        for row in rows:
            ...
            if 1 <= difficulty <= 5:
                distribution[difficulty] = row["count"]
